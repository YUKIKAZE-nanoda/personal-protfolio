                ...
                sm:w-[initial] sm:flex-nowrap sm:gap-5'>
                    {links.map(link => (
                        <motion.li key={link.hash} initial={{ y: -100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className='h-3/4 flex items-center justify-center relative ' >
                            <Link className={clsx("flex w-full items-center  justify-center px-3 py-3 hover:text-gray-950 transtion dark:text-gray-950 dark:hover", { "text-gray-950": activeSection === link.name })}
                                href={link.hash}
                                onClick={() => {
